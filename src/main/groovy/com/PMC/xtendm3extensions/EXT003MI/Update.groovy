 ...
 * 20230619   Hatem Abdellatif                Update MITTRA inventory accounting price Data (TRPR)
 * 20230829   Hatem Abdellatif                Adding more output fields:
 *                                            - message number (MSNR)
 *                                            - message line (MSLN)
 *                                            - sequence number (MSGS)
 */
  ...
  public void main() {
    ...
    def callback = {
      Map <String, String> response ->
        ...
        if(response.MSNR != null){
          messageNumber = response.MSNR;
        }
        
        if(response.MSLN != null){
          messageLine = response.MSLN;
        }
        
        if(response.MSGS != null){
          sequenceNumber = response.MSGS;
        }
    }
  ...
  private void updatePrice() {
    ...
    if (query.readAllLock(container, 6, updateCallBack)) {
      mi.outData.put("TRPR", "Successfully updated Price ${iTRPR}");
		  ...
		  mi.outData.put("CONO", "${currentCompany}");
		  mi.outData.put("MSNR", "${messageNumber}");
		  mi.outData.put("MSLN", "${messageLine}");
		  mi.outData.put("MSGS", "${sequenceNumber}");
		  mi.write();
		} else {
      mi.outData.put("TRPR", "Successfully updated Price ${iTRPR}");
		  mi.outData.put("RGTM", "Successfully updated Time ${entryTime}");
