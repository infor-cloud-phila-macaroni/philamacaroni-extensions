...
public class Add extends ExtendM3Transaction {
  ...
  private boolean validateOrderNumber(){
    boolean validRecord = false;
    Map<String, String> parameters = ["ORNO" : iORNO];
    Closure<?> handler = { Map<String, String> response ->
      if (!response.containsKey('errorMsid')){
        validRecord = true;
    ...
    miCaller.call("OIS100MI", "GetOrderHead", parameters, handler);
    return validRecord;
  }
  ...
  private boolean validFacility(){
    boolean validRecord = false;
    Map<String, String> parameters = ["FACI" : iFACI];
    Closure<?> handler = { Map<String, String> response ->
      if (response.containsKey('errorMsid')){
        validRecord = false;
      } else {
  ...
  /**
    ...
    * @return boolean
    */
  private boolean validateTimeStamp(String strDateTime){
    try {
